...
namespace LAppDefine {
    ...
    void  SetRenderSize(int w, int h) {
        RenderTargetHeight = w;
        RenderTargetWidth = h;
    }
    extern int callback_windowLoaded = 0;
    extern bool has_callback_windowLoaded = false;
    extern void SetCallBack_windowLoaded(int callback) {
        has_callback_windowLoaded = true;
        callback_windowLoaded = callback;
    }
    extern void CallBack_windowLoaded(int windowHwnd) {
        if (has_callback_windowLoaded) {
            function cb = (function)callback_windowLoaded;
            cb(windowHwnd);
        }
    }
    extern std::string firstModelName = "";
    extern void setFirstModel(std::string modelName)
    {
