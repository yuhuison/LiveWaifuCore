...
bool LAppDelegate::Initialize()
{
    ...
    DWORD exStyle = GetWindowLongA(GetActiveWindow(),GWL_EXSTYLE);

    exStyle |= (WS_EX_TRANSPARENT | WS_EX_LAYERED);
    SetWindowLongA(GetActiveWindow(), GWL_EXSTYLE,exStyle);
    LAppDefine::CallBack_windowLoaded((int)GetActiveWindow());
    if (_window == NULL)
    {
        if (DebugLogEnable)
        {
...
void LAppDelegate::OnMouseCallBackEx(double x, double y)
{
    _mouseX = static_cast<float>(x);
    _mouseY = static_cast<float>(y);
